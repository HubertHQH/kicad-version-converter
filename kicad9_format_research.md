# KiCad 9 vs KiCad 8 原理图文件格式差异研究报告（修订版）
...
KiCad 原理图文件 ([.kicad_sch](file:///c:/Data/AIProject/kicadversionparser/asset/kicad8/video/rams.kicad_sch)) 使用 S-expression 格式。文件头的 `version` 字段用 `YYYYMMDD` 日期格式标识。
...
输入: KiCad 9 .kicad_sch 文件
   ...
   │   AST 转换（9 条规则）        │
   │                              │
   │ R1: 修改 version/generator   │
   │ R2: pin_names hide 语法      │
   │ R3: pin hide 语法            │
   │ R4: 移除 embedded_fonts      │
   │ R5: sheet pin uuid 位置      │
   │ R6: 移除 sheet 新增属性       │
   │ R7: 移除 K9 特有元素          │
   │ R8: 元素排序（可选）          │
   │ R9: 序列化行长度控制          │
...
> - **符号库文件**: `.kicad_sym` 符号库文件也有版本差异，需要单独处理
